<template>
  <div class="w100-h100">
    ...
    <div v-if="saechaddress.length===0">
    ...
    <div v-else>
      <van-list>
        <van-cell v-for="(item) in saechaddress" :key="item.name"  :title="item.name" />
      </van-list>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    onSearch() {
      this.saechaddress = []
      Object.values(this.areaLists).map(item=>{
        Object.values(item).map(key=>{
            let indec = JSON.stringify(Object.values(key)).includes(this.value) 
            indec &&this.saechaddress.push(key)
        });
      })
      this.value= this.saechaddress.length===0?this.value:''
    },
    onCancel(){
      this.saechaddress = []
    },
