<template>
  <div>
    <div class="mag-10">
      ...
      <div>
        <van-row>
        <van-col >  <span class="font red">¥{{ details.present_price }}</span> </van-col>
          </van-row>
      ...
      <div>
        <van-row>
          ...
          <van-col span="8">收藏：
            <van-icon  v-if="isCollection===0" name="like-o" @click="collection"/>
            <van-icon class="red" v-else-if="isCollection>0" name="like-o" @click="delcollection"/>
            </van-col>
        </van-row>
      </div>
    </div>
...
export default {
  ...
  methods: {
    //删除收藏
    delcollection(){
       this.$api.getcancelCollection(this.details).then(res=>{
        this.$toast(res.msg)
      })
    },
    //收藏
