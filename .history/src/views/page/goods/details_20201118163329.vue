...
export default {
  ...
  methods: {
    //点击购买
    onSubmit() {
      this.$set(this.details,'num',this.num)
      this.$set(this.details,'total',this.total)
      this.utils.todata({url:'byorder',data:JSON.stringify(this.details)})
    },
    push() {
      this.show = true;
    },
  ...
  computed: {
    total() {
      return this.details.present_price * this.num * 100;
    },
    cartnum(){
