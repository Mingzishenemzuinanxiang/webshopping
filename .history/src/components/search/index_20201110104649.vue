  ...
  <div>
    ...
    <div v-show="viewsStatus">
      <van-list
        ...
        finished-text="没有更多了"
      >
        <van-cell v-if="list.length >0 " v-for="(item,index) in list" :key="item.name"><span v-html="item.nickname"></span></van-cell>
      </van-list>
    </div>
  </div>
</template>
  ...
  methods: {
    onSearch() {
      ...
      this.$api.getSearchList({ value: this.value }).then((res) => {
        if (res.code===200) {
          ...
          this.list.map(item=>{
            this.$set(item,'nickname',item.name.replace(`${this.value}`,`<span style="color:red">${this.value}</span>`))
          }) 
          this.addlishi()
        }
      });
    ...
    addlishi(){
      ...
      if(!lishi){
        ...
        JSON.parse(lishi)
        console.log(typeof lishi);
        lishi.push(this.value)
         localStorage.setItem('history',lishi)
      }
    },
