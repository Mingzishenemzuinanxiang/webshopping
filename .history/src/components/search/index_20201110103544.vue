  ...
  <div>
    ...
    <div v-show="viewsStatus">
      <van-list
        ...
        @load="onLoad"
      >
        <van-cell v-if="list.length >0 " v-for="(item,index) in list" :key="item.name"><span v-html="item.nickanme"></span></van-cell>
      </van-list>
    </div>
  </div>
</template>
    ...
    onSearch() {
      ...
      this.$api.getSearchList({ value: this.value }).then((res) => {
        if (res.code===200) {
          ...
          this.list.map(item=>{
            this.$set(item,'nickname',item.name.replace(`/${this.value}/g`,`<span style="color:red">${this.value}</span>`))
          }) 
