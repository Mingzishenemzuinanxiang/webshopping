<template>
  <div>
    ...
    <van-cell is-link @click="search">展示弹出层</van-cell>
   <van-popup v-model="viewsStatus" close-on-click-overlay="true" position="right" :style="{ height: '30%' ,width:'100%'}" />
    ...
    > -->
      <!-- <div v-show="viewsStatus">
        <van-row>
          ...
          <van-col span="4">
            <van-button v-show="history.length >0" icon="delete" type="warning" @click="delhistory"
          /></van-col>
        </van-row>
        <van-list
          v-model="loading"
        ...
        >
          <van-cell
            ...
            :key="item.name"
            > <span v-html="item.nickname"></span></van-cell>
        </van-list>
      </div> -->
    <!-- </van-popup> -->
  </div>
